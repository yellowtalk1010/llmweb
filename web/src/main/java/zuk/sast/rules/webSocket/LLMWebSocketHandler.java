...
import zuk.sast.rules.webSocket.llm.LLMPrompt;
import zuk.sast.rules.webSocket.llm.LLMReponse;
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        try {
            ...
            if(map.get("issueId")!=null && map.get("content")!=null){
                ...
                Set<String> set = DatabaseIssue.getAllIssue().stream().filter(issueDto -> issueDto.getId().equals(issueId)).map(dto->dto.getVtId()).collect(Collectors.toSet());
