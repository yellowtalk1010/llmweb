...
class MA3_3_Model extends IModel {
  ...
  override def run(days: List[ModuleDay]): Unit = {
    if(days.size>=5){
      ...
      val head = list.head
      if (
        new BigDecimal(list(4).change).compareTo(new BigDecimal(0)) < 0       //下跌
        ...
        && new BigDecimal(list(1).vol).compareTo(new BigDecimal(list(2).vol).add(new BigDecimal(list(3).vol).add(new BigDecimal(list(4).vol)))) > 0   //上涨并且，反包两日下跌交易量
        ...
        && List(list(4).low.toFloat,list(3).low.toFloat,list(2).low.toFloat,list(1).low.toFloat).sorted.head < list(0).low.toFloat
        //进入最低价不能跌破上3日最低价
