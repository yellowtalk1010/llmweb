      ...
      lines.asScala.foreach(line=>{
        try {
          ...
          if (jsonObj != null
            ...
            && jsonObj.get("stockType").toString.toUpperCase.equals(classOf[MA7_1_Model].getSimpleName.toUpperCase)) {
 ...
 * 底部放巨量
 */
class MA7_1_Model extends IModel {
  ...
  override def run(days1: List[ModuleDay]): Unit = {
    ...
    val ma7BlackTestMap = MA7_1_Model.modelBlackTestResultMap
...
//      )
  ...
  override def desc(): String = "底部放巨量（只看量）"
